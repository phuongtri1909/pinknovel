    ...
    <div class="container">
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    ...
                    <select id="month-filter" class="form-select">
                        ...
                        @for ($i = 1; $i <= 12; $i++)
                            <option value="{{ $i }}" {{ $i == date('m') ? 'selected' : '' }}>Tháng
                                {{ $i }}</option>
                        @endfor
                    </select>
                </div>
        ...
        <div class="card mt-4 mb-4">
            ...
            <div class="card-body">
                    ...
                    <table class="table table-hover align-middle">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th style="width: 15%" class="text-end"><i class="fas fa-coins me-1 text-muted"></i> Doanh
                                    thu</th>
                            </tr>
                        </thead>
                ...
                <div id="transaction-pagination" class="d-flex justify-content-center mt-3"
                                    ...
                                    <tr class="transaction-row">
                                        ...
                                        <td>
                                            <a href="{{ route('show.page.story', $story->slug) }}"
                                                class="text-decoration-none fw-semibold color-3">
                                                {{ $story->title }}
                                            </a>
                                        </td>
                                        <td class="text-end">
                                            <span
                                                class="fw-bold text-primary">{{ number_format($story->chapter_revenue) }}
                                                xu</span>
                                        </td>
                                        <td class="text-end">
                                            <span class="fw-bold text-warning">{{ number_format($story->story_revenue) }}
                                                xu</span>
                                        </td>
                                        <td class="text-end">
                                            <span class="fw-bold text-success">{{ number_format($story->total_revenue) }}
                                                xu</span>
                                        </td>
                                    </tr>
                                @endforeach
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        @if (count($topStories) > 0)
                            ...
                            <div id="top-stories-pagination" class="d-flex justify-content-center mt-3"
                                style="display: none !important;">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        @if (count($topChapters) > 0)
                                    ...
                                    <tbody id="top-chapters-list">
                                        @foreach ($topChapters as $index => $chapter)
                                            <tr>
                                                <td>{{ $index + 1 }}</td>
                                                <td>
                                                    <a href="{{ route('chapter', [$chapter->story_slug, $chapter->slug]) }}"
                                                        class="text-decoration-none color-3">
                                                        {{ $chapter->title }}
                                                    </a>
                            ...
                            <div id="top-chapters-pagination" class="d-flex justify-content-center mt-3"
                                style="display: none !important;">
...
@push('info_scripts')
    ...
    <script>
        ...
        function loadRevenueData() {
            const year = document.getElementById('year-filter') ? document.getElementById('year-filter').value : new Date()
                .getFullYear();
        ...
        function createOrUpdateChart(data, chartType) {
            ...
            if (!data.datasets || !data.labels || data.datasets.length === 0) {
                chartContainer.innerHTML =
                    '<div class="text-center py-5"><i class="fas fa-chart-bar fa-3x text-muted mb-3"></i><p>Không có dữ liệu doanh thu trong khoảng thời gian này</p></div>';
                return;
            }
        ...
        function updateRevenueSummary(data) {
            ...
            if (data.summary) {
                ...
                if (chapterRevenueElement) animateNumber(chapterRevenueElement, totalChapterRevenue);
                if (storyRevenueElement) animateNumber(storyRevenueElement, totalStoryRevenue);
            } else {
        ...
        function loadTransactionHistory(reset = false) {
            ...
            const transactionPeriodEl = document.getElementById('transaction-period');
            const yearFilterEl = document.getElementById('year-filter');
            const monthFilterEl = document.getElementById('month-filter');

            if (!transactionListEl || !yearFilterEl || !monthFilterEl) return;
            ...
            `;
                if (transactionPaginationEl) transactionPaginationEl.style.display = 'none';
            }
            ...
            if (transactionPeriodEl) transactionPeriodEl.textContent = month ? `${month}/${year}` : `Năm ${year}`;

            fetch(
                    `{{ route('user.author.revenue.transactions') }}?year=${year}&month=${month}&page=${currentTransactionPage}`
                )
                .then(response => response.json())
                .then(data => {
                    hasMoreTransactions = data.current_page < data.last_page;

                    if (transactionPaginationEl) transactionPaginationEl.style.display = hasMoreTransactions ? 'flex' :
                        'none';
                    ...
                    data.data.forEach((transaction, index) => {
                        ...
                        const userInitials = (transaction.user_name || 'U')
                            .split(' ')
                            .map(name => name[0])
                            .join('')
                            .substring(0, 2)
                        ...
                        <td>${contentHtml}</td>
                        <td class="transaction-amount text-shadow-custom">${(transaction.amount_received || 0).toLocaleString()} xu</td>
                    `;
        ...
        function loadTopStories(reset = false) {
            const storiesListEl = document.getElementById('top-stories-list');
            const storiesPaginationEl = document.getElementById('top-stories-pagination');
            const yearFilterEl = document.getElementById('year-filter');
            const monthFilterEl = document.getElementById('month-filter');

            if (!storiesListEl || !yearFilterEl || !monthFilterEl) return;
            ...
            `;
                if (storiesPaginationEl) storiesPaginationEl.style.display = 'none';
            }
            ...
            fetch(`{{ route('user.author.revenue.top-stories') }}?year=${year}&month=${month}&page=${currentStoriesPage}`)
                ...
                .then(data => {
                    ...
                    if (storiesPaginationEl) storiesPaginationEl.style.display = hasMoreStories ? 'flex' : 'none';
                    ...
                    data.data.forEach((story, index) => {
                        ...
                        <td>${story.purchase_count}</td>
                        <td>${(story.total_revenue || 0).toLocaleString()} xu</td>
                    `;
        ...
        function loadTopChapters(reset = false) {
            const chaptersListEl = document.getElementById('top-chapters-list');
            const chaptersPaginationEl = document.getElementById('top-chapters-pagination');
            const yearFilterEl = document.getElementById('year-filter');
            const monthFilterEl = document.getElementById('month-filter');

            if (!chaptersListEl || !yearFilterEl || !monthFilterEl) return;
            ...
            `;
                if (chaptersPaginationEl) chaptersPaginationEl.style.display = 'none';
            }
            ...
            fetch(
                    `{{ route('user.author.revenue.top-chapters') }}?year=${year}&month=${month}&page=${currentChaptersPage}`
                )
                .then(response => response.json())
                .then(data => {
                    hasMoreChapters = data.current_page < data.last_page;

                    if (chaptersPaginationEl) chaptersPaginationEl.style.display = hasMoreChapters ? 'flex' : 'none';
                    ...
                    data.data.forEach((chapter, index) => {
                        ...
                        <td>${chapter.purchase_count}</td>
                        <td>${(chapter.total_revenue || 0).toLocaleString()} xu</td>
                    `;
        ...
        function loadMoreStoryStats() {
            const storyStatsPaginationEl = document.getElementById('story-stats-pagination');
            ...
            fetch(`{{ route('user.author.revenue.story-stats') }}?page=${currentStoryStatsPage}`)
                ...
                .then(data => {
                    ...
                    if (storyStatsPaginationEl) storyStatsPaginationEl.style.display = hasMoreStoryStats ? 'flex' :
                        'none';
