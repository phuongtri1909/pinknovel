        ...
        <div class="story-info-body">
            ...
            <div class="tab-content" id="storyTabContent">
                ...
                <div class="tab-pane fade show active" id="edit" role="tabpanel" aria-labelledby="edit-tab">
                    <form action="{{ route('user.author.stories.update', $story->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="col-md-8">
                                ...
                                <div class="mb-3">
                                    ...
                                    <p class="text-muted small">Nhập các thể loại phân cách bởi dấu phẩy (,). Ví dụ: Tình cảm, Hài hước, Viễn tưởng</p>
                                    <input type="text" class="form-control @error('category_input') is-invalid @enderror" 
                                        id="category_input" name="category_input" value="{{ old('category_input', $categoryNames) }}" required>
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control @error('description') is-invalid @enderror" 
                                        id="description" name="description" rows="6">{{ old('description', $story->description) }}</textarea>
                                    @error('description')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('author_name') is-invalid @enderror" 
                                        id="author_name" name="author_name" value="{{ old('author_name', $story->author_name) }}" required>
                                    @error('author_name')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('story_type') is-invalid @enderror" 
                                        id="story_type" name="story_type" required>
                                        ...
                                        <option value="translated" {{ (old('story_type', $story->story_type) == 'translated') ? 'selected' : '' }}>Dịch/Edit</option>
                                        <option value="collected" {{ (old('story_type', $story->story_type) == 'collected') ? 'selected' : '' }}>Sưu tầm</option>
                                    </select>
                                    @error('story_type')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="mb-3">
                                    <label for="cover" class="form-label">Ảnh bìa {{ $story->cover ? '' : '<span class="text-danger">*</span>' }}</label>
                                    <input type="file" class="form-control @error('cover') is-invalid @enderror" 
                                        id="cover" name="cover" accept="image/*" {{ $story->cover ? '' : 'required' }}>
                                    @error('cover')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                    ...
                                    <div class="mt-3 cover-preview {{ $story->cover ? '' : 'd-none' }}">
                                        <div class="cover-container">
                                            <img src="{{ $story->cover ? Storage::url($story->cover) : '' }}" class="preview-cover" id="coverPreview">
                                ...
                                <div class="alert alert-warning mt-4">
                                    <div class="mb-2"><i class="fas fa-info-circle me-2"></i> <strong>Lưu ý:</strong></div>
                                ...
                                <div class="alert alert-info mt-4">
                                    <div class="mb-2"><i class="fas fa-info-circle me-2"></i> <strong>Lưu ý:</strong></div>
                                    <p>Truyện của bạn đang được xuất bản. Khi bạn chỉnh sửa thông tin, hệ thống sẽ gửi yêu cầu duyệt cho admin và những thay đổi sẽ được áp dụng sau khi admin phê duyệt.</p>
                                ...
                                <div class="alert alert-warning mt-4">
                                    <div class="mb-2"><i class="fas fa-clock me-2"></i> <strong>Đang chờ duyệt:</strong></div>
                                    ...
                                    <p><small>Thời gian gửi yêu cầu: {{ $story->latestPendingEditRequest()->submitted_at->format('H:i:s d/m/Y') }}</small></p>
                                </div>
                                @endif
                            </div>
                        </div>
                        ...
                        <div>
                            @if($story->chapters->count() < 3)
                                <span class="chapter-count-badge warning">
                                    <i class="fas fa-exclamation-triangle"></i> {{ $story->chapters->count() }}/3 chương tối thiểu
                                </span>
                            @else
                                <span class="chapter-count-badge success">
                                    <i class="fas fa-check-circle"></i> {{ $story->chapters->count() }} chương (Đủ điều kiện)
                                </span>
                            @endif
                        </div>
                        <div>
                            <a href="{{ route('user.author.stories.chapters.create', $story->id) }}#batch-upload" class="btn btn-outline-primary me-2">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @if($chapter->scheduled_publish_at && $chapter->scheduled_publish_at > now())
                                                <span class="badge bg-info ms-1" title="Hẹn giờ đăng: {{ $chapter->scheduled_publish_at->format('H:i d/m/Y') }}">
                                        ...
                                        <td>
                                            ...
                                            @else
                                                <span class="badge bg-warning text-dark">{{ number_format($chapter->price) }} coin</span>
                                            @endif
                                        </td>
                                        <td>
                                            <div class="btn-group btn-group-sm">
                                                <a href="{{ route('user.author.stories.chapters.edit', ['story' => $story->id, 'chapter' => $chapter->id]) }}" class="btn btn-outline-primary">
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-footer">
                                    ...
                                    <form action="{{ route('user.author.stories.chapters.destroy', ['story' => $story->id, 'chapter' => $chapter->id]) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                ...
                <div class="tab-pane fade" id="review" role="tabpanel" aria-labelledby="review-tab">
                    <div class="alert alert-info">
                        ...
                        <ul class="mb-0 mt-2">
                            <li class="mb-2">Truyện phải có đầy đủ thông tin cơ bản (tiêu đề, mô tả, thể loại, tác giả)</li>
                        ...
                        <div class="card-body">
                            @if($story->status == 'draft')
                                @if($story->chapters->count() < 3)
                                    <div class="alert alert-warning">
                                        ...
                                        <strong>Chưa đủ điều kiện:</strong> Truyện cần có ít nhất 3 chương trước khi gửi yêu cầu duyệt.
                                    </div>
                                @else
                                    ...
                                    <form action="{{ route('user.author.stories.submit.for.review', $story->id) }}" method="POST" class="review-form">
                                        @csrf
                                        <div class="mb-3">
                                            <label for="review_note" class="form-label">Ghi chú cho quản trị viên (không bắt buộc)</label>
                                            <textarea class="form-control" id="review_note" name="review_note" rows="4" placeholder="Nhập ghi chú hoặc thông tin bổ sung cho quản trị viên...">{{ old('review_note') }}</textarea>
                                            <div class="form-text">Bạn có thể để lại ghi chú cho quản trị viên về truyện của mình.</div>
                                        </div>
                            ...
                            @elseif($story->status == 'pending')
                                <div class="alert alert-warning">
                                    ...
                                    <strong>Đang chờ duyệt:</strong> Truyện của bạn đã được gửi đi và đang chờ quản trị viên phê duyệt.
                                </div>
    ...
    $(document).ready(function() {
        ...
        function detectChanges() {
            ...
            if ($('#title').val() !== originalData.title) {
                changes.push('<li>Tiêu đề: <span class="text-danger">' + originalData.title + '</span> → <span class="text-success">' + $('#title').val() + '</span></li>');
            }
            ...
            if ($('#author_name').val() !== originalData.author_name) {
                changes.push('<li>Tên tác giả: <span class="text-danger">' + originalData.author_name + '</span> → <span class="text-success">' + $('#author_name').val() + '</span></li>');
            }
            
            // Kiểm tra thể loại
            if ($('#category_input').val() !== originalData.categories) {
                changes.push('<li>Thể loại: <span class="text-danger">' + originalData.categories + '</span> → <span class="text-success">' + $('#category_input').val() + '</span></li>');
            }
            ...
            if ($('#story_type').val() !== originalData.story_type) {
                ...
                changes.push('<li>Loại truyện: <span class="text-danger">' + storyTypes[originalData.story_type] + '</span> → <span class="text-success">' + storyTypes[$('#story_type').val()] + '</span></li>');
            }
