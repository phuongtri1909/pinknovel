...
@section('info_content')
    ...
    <form action="{{ route('user.author.stories.chapters.update', ['story' => $story->id, 'chapter' => $chapter->id]) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-md-4">
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('number') is-invalid @enderror" 
                        id="number" name="number" value="{{ old('number', $chapter->number) }}" required>
                    @error('number')
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control @error('content') is-invalid @enderror" 
                id="content" name="content">{{ old('content', $chapter->content) }}</textarea>
            @error('content')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
            ...
            <div class="mb-3">
                ...
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="is_free" id="is_free_yes" value="1" 
                        {{ old('is_free', $chapter->is_free ? '1' : '0') == '1' ? 'checked' : '' }} onchange="togglePricingOptions()">
                ...
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="is_free" id="is_free_no" value="0" 
                        {{ old('is_free', $chapter->is_free ? '1' : '0') == '0' ? 'checked' : '' }} onchange="togglePricingOptions()">
            ...
            <div class="pricing-options" id="pricingOptions">
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('price') is-invalid @enderror" 
                        id="price" name="price" value="{{ old('price', $chapter->price ?? 5) }}" min="1">
                ...
                <div class="mb-3">
                    ...
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="has_password" id="has_password_yes" value="1" 
                            {{ old('has_password', !empty($chapter->password) ? '1' : '0') == '1' ? 'checked' : '' }} onchange="togglePasswordField()">
                    ...
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="has_password" id="has_password_no" value="0" 
                            {{ old('has_password', !empty($chapter->password) ? '1' : '0') == '0' ? 'checked' : '' }} onchange="togglePasswordField()">
                ...
                <div class="mb-3" id="passwordField" style="{{ !empty($chapter->password) ? '' : 'display: none;' }}">
                    ...
                    <div class="input-group">
                        <input type="password" class="form-control @error('password') is-invalid @enderror" 
                            id="password" name="password" value="{{ old('password', $chapter->password) }}">
                ...
                <div class="mb-3">
                    <label class="form-label d-flex align-items-center">
                        <input type="checkbox" class="form-check-input me-2" id="enableSchedule" 
                            onchange="toggleScheduleField()" {{ old('scheduled_publish_at', $chapter->scheduled_publish_at) ? 'checked' : '' }}>
                        Hẹn giờ xuất bản
                    </label>
