            ...
            <div class="card mb-4">
                <div class="card-header pb-0">
                    <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-2">
                        <h6 class="mb-0">Thêm cấu hình mới</h6>
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.configs.store') }}">
                        ...
                        <div class="form-group">
                            <label for="key" class="form-control-label">Khóa <span class="text-danger">*</span></label>
                            <input class="form-control @error('key') is-invalid @enderror" type="text" name="key" id="key" value="{{ old('key') }}" required>
                            @error('key')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <small class="form-text text-muted">Khóa là định danh duy nhất cho cấu hình</small>
                        </div>

                        <div class="form-group mt-3">
                            <label for="value" class="form-control-label">Giá trị <span class="text-danger">*</span></label>
                            <textarea class="form-control @error('value') is-invalid @enderror" name="value" id="value" rows="3" required>{{ old('value') }}</textarea>
                            @error('value')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="d-flex flex-column flex-md-row justify-content-end gap-2 mt-4">
                            <button type="submit" class="btn bg-gradient-primary btn-sm">
                                <i class="fas fa-save me-2"></i><span class="d-none d-md-inline">Lưu cấu hình</span><span class="d-md-none">Lưu</span>
                            </button>
                        </div>
                    </form>
                </div>
